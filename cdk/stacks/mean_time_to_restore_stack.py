...
"""
This contains an event bridge rule and lambda function to create mean_time_to_restore in an ops account
"""
...
class DoraOpsMeanTimeToRestoreStack(Stack):
    """
    This class contains an event bridge rule and lambda function to create mean_time_to_restore in an ops account
    """
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        lambda_role = iam.Role(
            self,
            "DoraMeanTimeToRestoreLambdaRole",
        ...
        lambda_func = lambda_.Function(
            self,
            "DoraMeanTimeToRestoreLambda",
            runtime=lambda_.Runtime.PYTHON_3_12,
            code=lambda_.Code.from_asset(
                os.path.join(
                    os.path.dirname(__file__),
                    "../lambdas/dora_custom_mean_time_to_restore_metric",
                )
            ),
            role=lambda_role,
        ...
        ops_event_bus = events.EventBus(
            self,
            "CentralizedEventBus",
            event_bus_name="MeanTimeToRestoreCentralizedEventBus",
        )

        event_rule = events.Rule(
            self,
            "DoraMeanTimeToRestoreRule",
        ...
        events.CfnEventBusPolicy(
            ...
            statement_id="MeanTimeToRestoreReceiverTooling",
        )
