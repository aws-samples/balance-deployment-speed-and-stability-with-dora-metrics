...
"""
This contains an event bridge rule and lambda function to create deployment_frequency in an ops account
"""
...
class DoraOpsDeploymentFrequencyStack(Stack):
    """
    This contains an event bridge rule and lambda function to create deployment_frequency in an ops account
    """
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        lambda_role = iam.Role(
            self,
            "DoraDeploymentFrequencyLambdaRole",
        ...
        lambda_func = lambda_.Function(
            self,
            "DoraDeploymentFrequencyLambda",
            runtime=lambda_.Runtime.PYTHON_3_12,
            code=lambda_.Code.from_asset(
                os.path.join(
                    os.path.dirname(__file__),
                    "../lambdas/dora_custom_deployment_frequency_metric",
                )
            ),
            role=lambda_role,
        ...
        ops_event_bus = events.EventBus(
            self,
            "CentralizedEventBus",
            event_bus_name="DeploymentFrequencyCentralizedEventBus",
        )

        event_rule = events.Rule(
            self,
            "DoraDeploymentFrequencyRule",
        ...
        events.CfnEventBusPolicy(
            ...
            statement_id="DeploymentFrequencyReceiverTooling",
        )
